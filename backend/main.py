...
# ─── App ──────────────────────────────────────────
app = FastAPI(title="Sacred KG API", version="2.0.0")
...
def verify_admin(
    ...
    """Проверка админ-доступа: JWT с role=admin ИЛИ admin-токен"""
    # Admin token header
...
async def admin_delete_place(place_id: str, _=Depends(verify_admin)):
    """Удалить место"""
    result = supabase.table("places").delete().eq("id", place_id).execute()
    return {"deleted": True}
...
async def add_favorite(place_id: str, user_id: str = Depends(verify_user)):
    try:
        result = supabase.table("favorites").insert({"user_id": user_id, "place_id": place_id}).execute()
        return {"added": True}
    except Exception:
...
@app.get("/")
async def root():
    return {"app": "Sacred KG API", "version": "2.0.0", "docs": "/docs"}
